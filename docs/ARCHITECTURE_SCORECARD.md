...
> Обновлено: **2026-09-13** · база замера: `1.27.101` · дерево: `main`
...
> Архитектура **68.5%**, долг **18%**. Карточки для сортировки — handoff Cursor.
...
| Нет слоя чтения над IDB | `DB.Workouts.getAll()` зовётся из 27 живых мест и каждый раз выгружает всю таблицу заново (`js/db/workouts.js:63`). Рендер Train платит это **на упражнение**, а не на экран (`PERF-HIST`). Цена растёт с историей пользователя, то есть полевым чеком на чистом телефоне не ловится. Симметрично A15: там views пишут в IDB, здесь — читают без кэша |
