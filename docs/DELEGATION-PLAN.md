...
| 2-5 | Различить SCORE vs DOTS (226==226) либо убрать дубль | P1 | M | 🔒 LEAD | `js/shared/athlete-room.js` | ✅ — Score=`athleteProScore` (composite), DOTS=`dotsScore` (чистый IPF) |
...
| L-2 | Аватар: цвет рамки (кольцо `--c1/--c2` было пустым → теперь от `avatar-color`); кроп-маска уже круглая; модалка кропа следует языку приложения | M | 🔒 LEAD | ✅ |
