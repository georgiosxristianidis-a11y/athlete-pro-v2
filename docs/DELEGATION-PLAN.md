...
| DI-7 | `#status-bar.workout-active` блюрил всю панель (`filter:blur(8px)`+opacity 0.4+`pointer-events:none`) → остров-child размывался/гас/не нажимался во время тренировки. → блюр снят с бара; рецессия только боковой хром (`.status-logo`/`.status-right`), остров всегда чёткий и интерактивный; убран дубль `status-bar-focused` | P0 | S | 🔒 LEAD | ✅ `2708947` |
...
**Плавность 60fps — сделано (`cb1513a`):** прогресс-бар отдыха = одна GPU-compositor-транзишн на всё время отдыха (re-arm только на старт/+время/catch-up), `update()` полностью пропускается во время отдыха. Ноль JS на кадр, без конфликтов.
