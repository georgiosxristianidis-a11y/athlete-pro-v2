...
export const AIOrchestrator = {
   ...
   */
   async streamResponse({ system, messages, images = [], engine = 'anthropic', onChunk }, req) {
    ...
    try {
      if (isGemini) {
        const model = gemini.getModel('gemini-1.5-pro-latest');
   ...
   */
  async generateJSON({ system, prompt, engine = 'anthropic' }, req) {
    const isGemini = engine === 'gemini';
    logInfo(req, 'ai_json_request', `Generating JSON with ${engine}`);

    try {
      if (isGemini) {
        const model = gemini.getModel('gemini-1.5-flash-latest');
