    ...
    try {
      if (isGemini) {
        ...
        if (customKey) {
           ...
           model = new GoogleGenerativeAI(customKey).getGenerativeModel({ model: 'gemini-1.5-pro-latest' });
        } else {
           model = gemini.getModel('gemini-1.5-pro-latest');
        }
  ...
  async generateJSON({ system, prompt, engine = 'anthropic', customKey }, req) {
    ...
    try {
      if (isGemini) {
        ...
        if (customKey) {
           const { GoogleGenerativeAI } = await import('@google/generative-ai');
           model = new GoogleGenerativeAI(customKey).getGenerativeModel({ model: 'gemini-1.5-flash-latest' });
        } else {
           model = gemini.getModel('gemini-1.5-flash-latest');
        }
