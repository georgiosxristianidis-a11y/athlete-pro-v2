...
export const REJECTED_LINES = [
  ...
  // Третья копия, найдена прополкой 2026-09-13: предупреждение двумя строками выше
  // сбылось буквально. Базы 1.27.50/51/52, три разных patch-id, ни одна не потомок
  // другой — реестр закрывает ровно те SHA, что в нём перечислены, поэтому копии
  // ищутся по содержимому (`js/readiness.engine.js` + `scripts/voice-lab-server.js`),
  // а не по имени ветки.
  {
    sha: '1e303a6147801d254b014b17e5298c210b19f6f8',
    name: 'claude/memory-index-f88f47',
    why: 'та же линия phase-8 на базе 1.27.52 — третья копия, движок готовности всё так же читает несуществующее w.date',
  },
];
