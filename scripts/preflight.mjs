...
//   3. база ветки уехала -> мёрж не будет FF / PR тянет чужие коммиты; на самой
//      main отставание считается тоже (FLOW-5) — из этого дерева читаются правила
...
if (!mainRef) {
  ...
  // FLOW-5. Имя ветки не говорит о свежести. Корневой чекаут стоит на main
  // почти всегда, и именно из него читаются правила — CLAUDE.md, .claude/rules,
  // скиллы. Отставшее дерево отдаёт отставшие правила, а старт сессии рапортует
  // «чист»: 2026-08-27 корень отстал на девять коммитов, preflight был зелёный,
  // и agent-brief прочитался без полей ФАЙЛЫ/ГЕЙТ — то есть закрытая карточка
  // выглядела открытой. WARN, а не FAIL: чинится одной командой, стопить сессию
  // из-за этого дороже, чем сказать вслух.
  const behind = Number(tryGit(['rev-list', '--count', `HEAD..${mainRef}`]));
  if (behind > 0) {
    add(
      'WARN',
      'база ветки',
      `main отстаёт от origin/main на ${behind} коммит(ов) — правила и скиллы в этом дереве протухли`,
      'git pull --ff-only origin main'
    );
  } else {
    add('OK', 'база ветки', 'сессия на main, дерево вровень с origin/main');
  }
