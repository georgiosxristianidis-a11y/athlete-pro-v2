...
import { computeVictims, parseFrontmatter, rewriteIndex } from './memory-core.mjs';
import { estimateTokens } from './check-docs-budget.mjs';
...
function archiveDest(memoryDir, victim, nowMs) {
  const ts = victim.frontmatter?.modified ? Date.parse(victim.frontmatter.modified) : victim.mtimeMs;
...
export function run({ dir, commit = false, nowIso = null, stdout = process.stdout } = {}) {
  ...
  if (victims.length) {
    ...
    for (const v of victims.slice().sort((a, b) => b.ageDays - a.ageDays)) {
      stdout.write(`  ${v.frontmatter?.type?.padEnd(9) || 'unknown'.padEnd(9)} ${String(Math.round(v.ageDays)).padStart(4)}д  ${v.file}\n`);
    }
    stdout.write('\n');
  }
