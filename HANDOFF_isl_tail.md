...
> **Баг A (0kg-сид) — ✅ УЖЕ ЗАКРЫТ** (верифай 2026-07-15): гард `canCompleteSet` (`c893251`, workout.store.js) в trunk И на проде — не-BW сет с 0 кг не закрывается + тост «укажи вес»; юнит-тесты `test/set-completion-guard.test.js`; негативный путь подтверждён живым репро в превью. Карточка из HANDOFF_island_bugs висела открытой только в памяти.
...
### L3 · ISL-DOT-AIRGAP — ✅ ЗАКРЫТА 2026-07-15 (ядро приехало с L2-полишем: deriveDotState на всех путях, airgap = нейтральная серая, idle 3px breathing; в этой сессии добиты хвосты — мёртвый CSS `.island-dot.online` удалён, доки sync-dot.js/тест-нейминг синхронизированы, спека S1 обновлена)
