...
describe('form', () => {
  ...
  test('prefill reaches back past the newest entry', () => {
    ...
    const entries = [
      e('2026-07-20', { waist: 82 }),
      e('2026-07-01', { waist: 86, hips: 97 }),
    ];
...
describe('trend', () => {
  ...
  test('history rows carry their own change, per position', () => {
    ...
    assert.equal(C.fieldDeltaAt(entries, 'waist', 2), null, 'у самой старой записи не с чем сравнивать');
  });

  test('series runs oldest → newest, so the sparkline reads left to right', () => {
    assert.deepEqual(C.cellSeries(entries, waist).map((p) => p.v), [86, 84, 82]);
  });

  test('entries are sorted defensively — storage order is not trusted', () => {
    const shuffled = [entries[1], entries[2], entries[0]];
    assert.deepEqual(C.sortEntries(shuffled).map((x) => x.date), entries.map((x) => x.date));
  });
});

describe('sparkline', () => {
  test('points stay inside the box, flat series included', () => {
    const pts = C.sparkPoints([80, 82, 81], 100, 38).split(' ').map((p) => p.split(',').map(Number));
    ...
    for (const [, y] of C.sparkPoints([80, 80], 100, 38).split(' ').map((p) => p.split(',').map(Number))) {
...
describe('body fat', () => {
  test('Navy formula answers only when it has the inputs', () => {
    ...
    assert.equal(C.bodyFatNavy({ sex: 'f', heightCm: 170, waistCm: 74, neckCm: 32 }), null, 'женская формула требует бёдра');
  });
  ...
  test('enrich fills the derived value so the tile can show a trend', () => {
    ...
    assert.ok(rows[0].body_fat > 0 && rows[1].body_fat > rows[0].body_fat, 'уже талия — ниже процент жира');
...
describe('view wiring', () => {
  ...
  test('a tile opens the whole form, focused — not a one-field modal', () => {
    assert.match(src, /data-action="bs:edit" data-focus="\$\{cellFocusField\(cell\.id\)\}"/,
  ...
  test('a partial update does not blank the other tiles', () => {
    ...
    assert.match(src, /points\[points\.length - 1\]\.v/,
  ...
  test('the re-render after save keeps the host it was mounted into', () => {
    ...
    assert.match(src, /_root = root/, 'хост не запоминается — ре-рендер в комнате атлета потеряет цель');
    assert.match(src, /targetEl \|\| _root \|\| document\.getElementById/);
  });

  test('an empty input means "leave as is", not "erase"', () => {
    assert.match(src, /if \(raw === ''\) continue;/,
