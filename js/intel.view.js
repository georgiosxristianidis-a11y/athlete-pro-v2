  ...
  async function speakText(textToSpeak, slot) {
    ...
    try {
      ...
      /* Волну ЖДЁМ, хотя она всего лишь картинка. `createMediaElementSource` уводит
         ...
         теряет вывод. Безопасный порядок один: сперва граф, потом play().
         Ожидание ограничено потолком внутри `startVoiceWave` (ATTACH_TIMEOUT_MS):
         на заблокированном автоплее `ctx.resume()` не резолвится до жеста, и
         безусловный await подвесил бы речь вместе с `_isSpeaking` навсегда. */
