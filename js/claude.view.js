...
export const Claude = (() => {
  ...
  async function renderFAB() {
    ...
    if (content) {
      if (videoMode) {
        content.innerHTML = `<video id="claude-fab-video" class="fab-video" autoplay loop muted playsinline preload="metadata" src="${PANDA_VIDEO_SRC}" poster="${PANDA_POSTER_SRC}" aria-hidden="true"></video>`;
      } else {
