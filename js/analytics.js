/* ════════════════════════════════════════════════════════
   analytics.js — Athlete Pro  |  Block 5
...
const Analytics = (() => {
  ...
  function _renderQuickStats(workouts) {
    ...
    _set('an-total-vol', _fmtVol(totalVol) + '<span class="stat-chip-unit">kg</span>');
    ...
    DB.Workouts.weeklyTrend(10).then((buckets) => {
      ...
      buckets.forEach((b, i) => {
        ...
        if (b.tonnage > 0) {
          ...
          ctx.fillText(_fmtVol(b.tonnage), x + bw / 2, y - 4);
        }
      });
    });
  }

  function _weekLabel(bucket) {
  ...
  function _renderPPLDonut(workouts) {
    DB.Workouts.pplTonnage().then((ppl) => {
      ...
      ctx.fillText(total > 0 ? _fmtVol(total) : '—', cx, cy - 7);
      ctx.fillStyle = 'rgba(90,96,112,0.8)';
      ctx.font = `600 9px -apple-system, sans-serif`;
  ...
  function _renderTimeChart(workouts) {
    DB.Workouts.weeklyTrend(8).then((buckets) => {
      ...
      data.forEach((d, i) => {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(_weekLabel(d), x + bw / 2, 120 - 4);
