...
const Workouts = {
  ...
  getAll() {
    return getAll(S.WORKOUTS).then(list =>
  ...
  weeklyVolume() {
    ...
    return this.getAll().then(list =>
      list
        .filter(w => w.timestamp >= since)
        .reduce((sum, w) => sum + (w.tonnage || 0), 0)
    );
  },
  ...
  monthlyCount() {
    ...
    return this.getAll().then(list =>
      list.filter(w => w.timestamp >= from).length
    );
  },
...
const Settings = {
  ...
  set(key, value) {
    return tx(S.SETTINGS, 'readwrite').then(s =>
      req2p(s.put({ key, value }))
    );
  },

  get(key, fallback = null) {
    return tx(S.SETTINGS).then(s =>
      req2p(s.get(key)).then(r => (r ? r.value : fallback))
    );
  },
...
const Events = {
  ...
  getAll() {
    return getAll(S.EVENTS).then(list =>
      list.sort((a, b) => b.timestamp - a.timestamp)
    );
  },
