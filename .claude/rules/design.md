...
- Типографика — шкала `--fs-*` / `--fw-*` в `css/base.css :root` (решение TYPE-1, 2026-07-28). Сырые px в `font-size` и цифры в `font-weight` запрещены.
  ...
  - Переведены все файлы `css/`, включая `intel.css`. Проверено аудитом 2026-08-20: вне `base.css` осталось 2 сырых `font-size` (SVG-текст, `base.css:1675-1676` — карточка DS-3) и ноль цифровых `font-weight`. Инлайны в `js/` — узаконенное исключение `pip.js` (чужой документ).
