...
- Типографика — шкала `--fs-*` / `--fw-*` в `css/base.css :root` (решение TYPE-1, 2026-07-28). Сырые px в `font-size` и цифры в `font-weight` запрещены.
  ...
  - Переведены `base` · `dashboard` · `analytics` · `workout` · `athlete-room` · `profile` (+ `js/profile.view/*`) · `journal` · `privacy` · `summary` · `dynamic-island` · `claude`. Остаток — `intel.css` и инлайны в `js/`: карточка TYPE-2, см. `docs/handoff/HANDOFF_design_system.md`.
