...
## ✅ МЁРТВЫЙ ВЕС МЕДИА — panda-idle выпилен (2026-07-26, `5293074`)
...
- **Гоча (важнее самого удаления):** прекеш `sw.js` их всё ещё тянул — 2.7 МБ из ~3.11 МБ медиа-веса. Удалить файл из `assets/` **без** регенерации манифеста = 404 в `cache.addAll` → install SW падает целиком, обновления не встают. Порядок обязателен: `git rm` → `npm run build:sw` → коммит одним куском. Здесь: 117 ассетов, CACHE_NAME `v114-2549f128` → `v114-f6e669b0`.
- ⚠️ **Конфликт-риск для LEAD:** F-7 (PR#36, ремонт медиа-веса прекеша) правит тот же блок `ASSETS` в `sw.js`. Сводить — не руками: взять любую сторону и перегенерить `npm run build:sw`, дальше сверять по числу ассетов, не по диффу.
- Гейт: `npm test` 308/308 · eslint 0 err. `stylelint` в worktree не резолвится (известная гоча без `node_modules`) — CSS не менялся, вторая половина `npm run lint` формально не отработала.
