...
- ✅ **Q1** Zero-friction prefill — **DONE (сверено 2026-06-25)**. `buildSession()` (`js/workout.store.js`) сидирует `set.weight` из прошлой сессии: case 1 (программа) через `prog.getProgression`, case 2 (free) через `_smartNextWeight` (повтор последнего веса либо +2.5 при взятой цели/лёгком RPE). `selectType` грузит историю из БД и передаёт `{ workouts }`. Барабан читает `data-value=set.weight`. Проверено живьём: last 92.5×8 @rpe8 → старт 95. Остаток (опц., НЕ обязателен): reps берутся из шаблона плана, не из факта прошлой сессии.
...
- ⬜ **D2** Декаплинг (роадмап Фаза 2) — подтвердить: **инкрементально за флагами** (`js/flags.js`), НЕ big-bang (внешняя критика «100% месяца на переписку» — отклонена как продукт-риск). **1-й strangler-таргет = safe-by-default рендер:** тег-шаблон `` html`...` `` с авто-`esc()` интерполяций (raw = явный opt-in) в `js/shared/utils.js`; новые/тронутые view переводим на него, легаси живёт. Закрывает класс «забыл esc» (единственное полностью валидное замечание критики). Параллельно — продолжать выносить data-shaping из `render.js` в сторы (как `buildSessionSummary`).
- ⬜ **D3** опц. source-fix VT-abort: гард на pending `startViewTransition` в `shell.js:54` `Nav.go` (быстрая навигация рвёт незавершённый transition → benign InvalidStateError; boundary уже глушит, но можно в источнике).

### 📦 БЭКЛОГ — осознанно НЕ на завтра (анти-overscope)
Завтра реалистично = только Фаза M (merge — полноценная сессия). Ниже — отложено намеренно, тащить НЕ надо:
- API-contracts: zod/safe-parse на ОТВЕТАХ AI (`generateJSON`) — дёшево, низкий приоритет.
- Signals/Proxy state — только при реальной боли ре-рендера (YAGNI).
- Выпил `unsafe-inline` целиком — XL-хвост Strangler, идёт постепенно ПОСЛЕ внедрения `` html`` `` (D2), не отдельной задачей.
- Роадмап Фаза 5 (полиш: WAAPI-spring, kinetic-typography) — после стабильных 60fps.
...
2. **Strangler-Fig.** Не сносим легаси. Новый код — рядом (`js/v2/` или новый модуль) за флагом, легаси работает 100%. Переключаем по одному микро-элементу, коммитим зелёным.
3. **Trunk-based (ветки < 24ч).** Задача дня = микро («перевести кнопку save на event-delegation»), не «переписать Workout». Застрял > 24ч → `git checkout .`, дроби на два. Длинные ветки = долг (см. ниже — текущая ветка перегружена, мёрджить в byoi).
4. **Safety net:** перед крупным — тег `checkpoint-<date>`; CI (`.github/workflows/ci.yml`: lint+test+audit+e2e) + pre-push hook (audit+lint) — зелёный гейт обязателен. SW-манифест авто: `npm run build:sw` (НЕ руками), затем бамп `CACHE_NAME`.
...
**BUG-тикеты:**
- ✅ **BUG-2** `window._coreCheckedState` → `State.coreChecked` — **DONE (сверено 2026-06-25)**, `handlers.js:486`/`render.js:163` читают стор, глобала в коде нет (`render.js:105`).
- ✅ **BUG-3** Layout thrashing `_initDrag` — **DONE (сверено 2026-06-25)**, rects кешируются на pointerdown + мутации в одном rAF (`render.js:595`).
...
- ✅ **BUG-3** — Layout Thrashing в `_initDrag` — DONE (rect-cache + rAF, `render.js:595`).
- ✅ **BUG-2** — `window._coreCheckedState` → `State.coreChecked` — DONE (`render.js:105`).

### Фаза 3 — W-1 Add Exercise Live — ✅ DONE (сверено 2026-06-25)
Реализовано: `_addLiveExercise()` (`handlers.js:702`), кнопка `wo:addLiveEx` (`render.js:425`), пикер `openExercisePickerModal`, синтетический блок `custom`, флаг `isAdded`→IDB для раздельной аналитики.
...
> 🔴 **СВЕРКА 2026-06-25 (Opus, LEAD):** весь LEAD-кодовый бэклог §9/§Q закрыт по факту кода — НЕ переделывать:
> **Phase B** (sync dot) ✅ · **Q1** (drum prefill) ✅ · **BUG-2** (`State.coreChecked`, `render.js:105`) ✅ · **BUG-3** (rect-cache+rAF в `_initDrag`, `render.js:595`) ✅ · **W-1** Add Exercise Live (`_addLiveExercise`, `handlers.js:702`) ✅.
> **Реально открыто:** делегируемое — Phase D spacing-sweep (🟩 Gemini), Phase E `--c-accent`→`--c-chrome` sweep (🟩/🟦), A-4 тест барабана (🟦 Sonnet); за пользователем — DS1/DS2 (полевой CRDT 2-девайс + миграция IDB), гигиена `.gitignore` UTF-16 + `gitleaks.exe` untracked.
...
### Phase B — ✅ DONE (сверено 2026-06-25)
Sync Indicator в Island полностью реализован. `deriveDotState()` (`js/shared/sync-dot.js`) — 6 состояний (synced/syncing/error/offline/no-cloud/airgap); CSS dot-состояния в `dynamic-island.css:251-272`; остров слушает `ap-sync-status`+online+`ap-privacy-mode` и зовёт `_updateNetworkStatus()`; `SyncManager` шлёт `ap-sync-status`; `window.__cloudConfigured` в `supabase.js`. Юнит-тест `test/sync-dot.test.js` (8/8). Закрывающий фикс `910f66c` (2026-06-25): idle-ветка `update()` тоже идёт через `deriveDotState` (была бинарная `online`/`offline` без CSS). B-1/B-2/B-3/B-4 — все done.
