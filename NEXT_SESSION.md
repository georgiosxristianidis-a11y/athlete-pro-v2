...
> **Сторож пока НЕ работает — не полагаться на него.** Поставлен в 1.27.23 и мёртв с того
> же момента (403 на запросе связанных PR валил job целиком, вместе с обоими сигналами).
> Фикс готов в ветке, не влит; плюс у сторожа найдено слепое пятно — коммит, на котором CI
> не запускался ВООБЩЕ, он не видит по построению. Пока это не закрыто, единственный барьер
> у `main` — `pre-push` и дисциплина. Детали и карточки — `docs/handoff/HANDOFF_main_watchdog.md`.
...
| `docs/handoff/HANDOFF_main_watchdog.md` | **Сторож `main` мёртв, фикс не влит.** WATCH-1 — добить живой проверкой (нужен прогон, не рассуждение); WATCH-2 — слепое пятно: коммит без единого прогона CI сторож не видит |
