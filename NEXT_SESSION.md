...
> **Дизайн-система (заведена 2026-07-27, активна):** `docs/handoff/HANDOFF_design_system.md` — иерархия · отступы · движение · отклик. Очередь: ANIM-1/ANIM-2 ✅ · TYPE-1 ✅ (полевой вердикт Gio: «читается хорошо») · PP-6 ✅ · PP-4 ✅ (1.25.34) → TYPE-2 → SPACE-1 → GUARD-1; отдельно PP-3 (ждёт двух решений Gio).
> **Полевой отчёт Gio 2026-07-29 разложен на карточки — `docs/handoff/HANDOFF_next_cards.md` § ПОЛЕ-2026-07-29.** PROF-1 закрыта (1.25.35), следующая — **LOG-1: журнала тренировок в приложении нет вообще**, прошлую тренировку нельзя открыть ни с одного экрана. Дальше AN-1/AN-2/AN-3 (аналитика), PLAN-1 (у редактора плана 7 классов без CSS), OVW-1, PCT-1. Там же разбор сессии 2026-07-27 и открытые вопросы к Gio. Профиль-паспорт air/отступ до шкалы (не начато, ждёт уточнения элемента у Gio): `docs/handoff/HANDOFF_profile_hero_spacing.md`.
