...
> реально завёл issue. **Но он не всесилен:** коммит, на котором CI не запускался ВООБЩЕ,
> сторож не видит по построению — он висит на `workflow_run` от CI. Карточка WATCH-2 открыта,
> детали — `docs/handoff/HANDOFF_main_watchdog.md`.
...
| `docs/handoff/HANDOFF_main_watchdog.md` | Сторож `main`. WATCH-1 закрыта (1.27.26, проверена живыми прогонами). **Открыта WATCH-2** — слепое пятно: коммит, на котором CI не запускался вообще, сторож не видит |
