...
## 🔶 AIR-4 — код готов, ждёт полевой OLED-чек (2026-07-26, Sonnet, НЕ ВЛИТО)

Ветка `air-4-sweep-guard` (от trunk `f1cbf67`), коммиты `2977a49` + `a113cca`. Детали — `HANDOFF_air_refactor.md` § AIR-4.

- Sweep `backdrop-filter` по css/ — нарушений вне Tier-2 вайтлиста не найдено (дерево уже чистое).
- Новый тест-гард `test/air-tier-guard.test.js` фиксирует вайтлист гейтом (проверен инъекцией нарушения).
- Пункт «DESIGN.md синхронизировать» закрыт без кода — решение Gio, DESIGN.md уже архивирован, спек в `CLAUDE.md`.
- SW-бамп сделан (`build:sw` → digest `9d90174c`).
- Гейт: unit **309/309** (+1), lint 0 err.

**Остаток:** финальный полевой OLED-чек всех экранов — за Gio. После него DoD-2 закрыт (вместе с AIR-2b).
