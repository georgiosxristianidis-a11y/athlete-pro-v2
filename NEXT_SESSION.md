...
- Свободно агенту: **AGENT-10** — авточасть **LAUNCH-10** и **HYG-7** сданы 19.09
- **AGENT-10 (19.09) — блокер очереди:** `npm audit` отвечает 400, SCA в `pre-push` и в шаге CI «Security audit» читает это как High/Critical → красный любой PR. Хендофф, § AGENT-10
...
- Закрытые карточки в роутере не перечисляются — `CHANGELOG.md` и хендоффы; открыто ровно то, что выше
