...
> Обновлено 2026-08-30 (VOICE-1, 1.27.85). Осталось поле: Gio + две карточки VOICE/MD
...
- Свободно агенту: **LAUNCH-9** пункт 7 — F-7/F-8 (молчаливые дефолты пола и приватности)
- Свободны и не блокируют трек: **MD-1** (формат ответа под движок) и **VOICE-2** (визуализация озвучки, код в невлитом `f7fdb95`) — § «Линия VOICE»
- Свободны также: **AGENT-7/8**, **HYG-5** (83 невлитых ветки в WARN). **AGENT-4** ждёт окно до 08.09 (`unsigned` = 6)
- Закрыты: LAUNCH-1/2, **3A**, **3B**, **4**, **5A**, **5B**, **6**, **7**, **8**, **9 pts 3–6 (F-10, F-11)**, **PC-1**, **PC-2** (линия PANDA Core в архиве), **VOICE-1**, AGENT-1/2/3/5/6, **FLOW-5**
