...
> Обновлено 2026-08-27 (LAUNCH-3B закрыта в PR #269, 1.27.77; блокировка LAUNCH-5B снята).
...
- **Следующая — LAUNCH-6** (ИИ-тренер): ключи заводит Gio, задержка снаружи; закрывает F-12/F-13
- Затем **LAUNCH-5B**, пункты 3–4 **LAUNCH-9** (F-6, F-9), **LAUNCH-4** (счётчик, PR #243), 7/8/10
- **Параллелить 5B с 6 и 9 нельзя** — пересечение по `claude.view.js` и `onboarding.js`
- Свободны также: **AGENT-7/8**, **HYG-5**. **AGENT-4** ждёт окно наблюдения до 08.09 (`unsigned` = 6)
- Закрыты: LAUNCH-1/2, **3A**, **3B**, **5A**, AGENT-1/2/3/5/6, **FLOW-5**
