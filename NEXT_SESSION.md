...
>
> **🟢 ЗА GIO, ОДИН ЗАХОД — `docs/handoff/HANDOFF_field_check.md` (заведён 2026-08-01).** 11 карточек с зелёным гейтом и кодом на проде висят неподписанными по DoD-ступени 5. Собраны в два списка: **А — дома, ~5 мин, до зала** (экспорт бэкапа первым как страховка · сплеш · офлайн-панда · журнал · скорость профиля) и **Б — в зале по ходу тренировки** (барабан · rest-кнопки · PiP-«далее» · пилюля отдыха · OLED в темноте · маскот). Перед заходом включить два флага: `fab-video` (Профиль → AI) и `drum-window` (консоль) — при вердикте «ок» станут дефолтом ON. Заход закрывает DoD-2 и DoD-5 целиком и открывает счёт серии DoD-1 (сейчас 0/10).
>
> **Разбор долгов 2026-08-01 (Opus 5) — блок «заблокировано Gio» расчищен целиком, 9 → 0.** PP-3 разблокирована (делать целиком + ячейку DOTS убрать тем же заходом) · O-7 решён паллиативом без билд-шага · db-backup-ветка закрыта без вливания (ветку не сносить до 2026-08-28 — правило 30 дней) · `stash@{1}` корня снесён после сверки с main · `reicon` — вопрос снят фактом (зависимости не существует) · кнопка «назад» → карточка NAV-BACK-1 · profile-hero-spacing свёрнут в SPACE-1 (хендофф в `docs/_archive/`) · красная island-dot → в полевой чек-лист · W (ревизия правил) отложена отдельной сессией.
>
> **Порядок взятия (утверждён Gio, минимум конфликтов по файлам):** TYPE-2 (`dashboard.css` — ни с чем не дерётся) → **PP-3** (профиль + Athlete Room) → **SPACE-1** (`profile.css`, вбирает profile-hero-spacing) → **NAV-BACK-1 + DEAD-1** (одно окно на двоих: общий `base.css`, отдельные коммиты) → GUARD-1 (закрывающая). PP-3 строго ДО SPACE-1: она меняет состав бенто, ритм выравнивается по финальному составу.
...
> **Полевой отчёт Gio 2026-07-29 разложен на карточки — `docs/handoff/HANDOFF_next_cards.md` § ПОЛЕ-2026-07-29.** Закрыты: PROF-1 (1.25.35) · LOG-1 (1.25.38) · PLAN-1 (была закрыта в main до прочтения карточки — не брать) · PCT-1 (1.25.55) · **AN-1** (1.25.61, выбор периода в Stats) + попутный фикс Strength Index hero (1.25.62 — срок «1y» вместо честных месяцев на журналах короче года). Следующая — **AN-2: разрез по упражнению** (тап открывает историю лифта — рабочие веса/1ПМ/объём во времени), затем AN-3 (время в зале, за тот же период — инфраструктура периода из AN-1 уже есть в `analytics.store.js` `periodRange()`), OVW-1. Там же разбор сессии 2026-07-27 и открытые вопросы к Gio. Профиль-паспорт air/отступ до шкалы (не начато, ждёт уточнения элемента у Gio): `docs/handoff/HANDOFF_profile_hero_spacing.md`.
...
- ~~UI-кнопка «назад» (Apple-паттерн, top-left шеврон на сабэкранах)~~ — дизайн обсуждён с Gio 2026-08-01, заведена полноценной карточкой **NAV-BACK-1** в `HANDOFF_next_cards.md` (единым компонентом сразу на все подэкраны; не параллелить с TYPE-2/SPACE-1 — общий `base.css`).
- **DEAD-1** (заведена 2026-08-01, `HANDOFF_next_cards.md`) — мёртвый CSS `base.css` одной карточкой вместо трёх хвостов: свип дал 9 классов без потребителя + мёртвый `renderChamberPill` со всем блоком `.cp-*`; `--t-slow` и `toggleReminder` при сверке оказались уже удалёнными, `.skip-link` — не мёртвый CSS, а потерянная a11y-фича (решение Gio). Берётся тем же окном, что NAV-BACK-1.
