...
| `docs/handoff/HANDOFF_panda_core.md` → **GUARD-1** | **Разбор линии `feature/elite-hud-wow` (Antigravity), 2026-08-11. Линия НЕ вливается** — утечка BYOK-ключа в неаутентифицированный эндпоинт, синк мимо `airgap`, снесённый `SyncManager`, ослабленный гард `air-guard`. Код сохранён тегом `checkpoint-elite-hud-wow`. Из линии извлекается только UI экрана `s-intel`: GUARD-1 (LEAD) → HYG-3 → HUD-1..3 (worker) → HUD-4 (LEAD). Параллелить HUD нельзя — общий `js/intel.view.js`. Решения Gio закрыты грилежом, переоткрывать не нужно |
...
инлайнов в `js/`. Дальше **GUARD-1** (закрывающая:
