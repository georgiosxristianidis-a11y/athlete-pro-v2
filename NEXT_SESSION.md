...
### 🔜 Остаток (делегировано пользователем в Antigravity)
- 🟩 **GEMINI** (перезапустить с чистым ТЗ — раньше «done», по факту НЕ сделано): 1-5 (`claude.css` FAB rgba), 1-6 (`#000`→токены), 2-6 (`toFixed`→`format.js`), BUG-6 (rgba в `workout.css`/`athlete-room.css`), 5-6 (онбординг прогресс-бар). LEAD сделает код-ревью после.
