...
| Хендофф | Что внутри |
|---|---|
| `docs/handoff/HANDOFF_boot_brand.md` → **BRAND-PACK** | **Активная фаза, следующая карточка.** Запуск и знак: три поколения логотипа жили одновременно, сплэш закрывал уже отрисованный шелл. CLEAN-1 закрыта (1.27.34). Дальше BRAND-PACK → SPLASH-1 → MOTION-1, порядок утверждён Gio 2026-08-08. Параллелить нельзя: три из четырёх карточек трогают `index.html`, две — `dashboard.js` |
