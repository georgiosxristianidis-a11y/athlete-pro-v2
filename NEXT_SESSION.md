...
- **DOCS-2** — урезка `MEMORY.md`: закрытое из индекса вон, хук ≤80 символов. Вынос узких уроков — только после проверки, всплывают ли 49 файлов-сирот отзывом.
...
- **DOCS-4** 🔴 — гард бюджета считает `\r`: на Windows (`core.autocrlf`) красный с 1.27.52 при зелёном CI на Linux. Нормализовать в `estimateTokens`, потолок не поднимать. Заодно нет записи 1.27.52 в `CHANGELOG.md`.
