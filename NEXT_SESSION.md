...
> Обновлено 2026-09-15 (EX-RU-1 закрыта 1.27.113; A2 влита). Осталось поле: Gio
...
- Свободно агенту: **LAUNCH-10** (сверка чеклиста; п. 7 — `npm run smoke:prod`) + **HYG-5**, **AGENT-9** (разблокирована закрытием HYG-6)
- **Префикс ветки — гард с 15.09 (AGENT-8):** имя сверяется с трейлером в pre-push и CI. Ветка без `cursor/`/`claude/` — блок; живые переименовать (хвост HYG-5)
- **Архитектурный трек:** фазы 0-4 — хендофф Cursor § «Порядок работ». Фазы 0-2 закрыты. Фаза 3: `A1`+`A8`+`A15`+`A2` закрыты → следующая `A5` → `A9`/`A13`
...
- Закрыты: LAUNCH-1/2, **3A**, **3B**, **4**, **5A**, **5B**, **6**, **7**, **8**, **9 pts 3–7 (F-10, F-11, F-7/F-8)**, **PC-1**, **PC-2**, AGENT-1/2/3/**4**/5/6/**7**/**8**, **FLOW-5**, **BOOT-TRIM**, **VOICE-2**, **HYG-6**, **EX-RU-1**, **AI-1**
