...
- **Вердикт «линию не вливать» = строка в `REJECTED_LINES`** (`scripts/rejected-lines.mjs`) тем же PR, что и сам вердикт. Якорь — SHA, не имя ветки или метки: ref переименовывают, удаляют и забывают запушить, а на одном отвергнутом коммите легко висит девять имён. Гард гоняется в `preflight`, pre-push и CI; молчит сам, если линия однажды легально приедет в main. Обход осознанный: `DONOR_OK=1 git push`. Список, который пополняется памятью, — не гард (BASE-1).
- **Правила и гарды, живущие в чекауте, дрейфуют вместе с чекаутом.** `core.hooksPath` относительный (`.githooks`) и резолвится от корня КАЖДОГО рабочего дерева — протухший корень больше не выключает хуки у всех разом молча (HYG-4 п. 1); `preflight` краснеет, если путь резолвится вне чекаута. Протухший `CLAUDE.md` = протухшие правила. Единственный недрейфующий слой — CI и настройки GitHub, поэтому дубль гарда в CI обязателен. Агент ведёт себя странно → сперва `git log origin/main -- CLAUDE.md` и сверка его базы.
