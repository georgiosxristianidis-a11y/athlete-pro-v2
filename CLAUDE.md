...
- **FF-only мёрж:** в trunk только `git merge --ff-only <ветка>`. Отказ FF = не ошибка, а сигнал «trunk уехал»: `git rebase <trunk>` в своей ветке → гейт → снова FF. Ветка — всегда от свежего trunk (сверить хеш с NEXT_SESSION.md), вливать сразу после зелёного гейта, не копить.
