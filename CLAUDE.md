...
- **`CLAUDE.md` — не больше 200 строк.** Разрослось → выносить детали в `docs/RULES.md` / `.claude/rules/*.md` / хуки, здесь оставлять только имя файла и одну строку-ссылку, не пересказ.
...
- **Verify-over-trust:** любое «готово» (вкл. Gemini) — гипотеза, пока gate-команда != 0. Показывать фактический вывод запущенной команды: нет лога — значит не выполнено, а фантазия.
- **Причина, потом ошибка:** сперва искать root cause, потом чинить — патч симптома вслепую всплывает тем же багом на следующем цикле.
