...
- **Счёт тестов на 1.25.35 — 390.** Прогнал и получил заметно меньше — это не «зелёный», это сигнал, что ветка не содержит половины сюиты (кейс PP-6: 211/211 на базе от 28 июня). Сверяться: `git show origin/main:package.json` + прогон на свежем `main`.
