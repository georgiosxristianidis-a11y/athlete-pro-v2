# HANDOFF — стек карточек (после Air Cleanup 1-3/5/6)

> Обновлено 2026-07-05 (реконсиляция после MERGE-QUEUE-2). Правило: 1 карточка = 1 сессия, сверху вниз.
> Гейт перед коммитом: `npm test` (229) + `npm run lint` (0 err). Перед рискованным — тег `checkpoint-<date>`.
> База: trunk `claude/csp-soft-delete` = main = origin/main = **`bb980c8` (релиз 1.20.0, SW v92, следующий свободный v93)** — включает blur-фикс, CORE-TAP, статбар, STATS-1. Проверять принадлежность коммита линии через `git merge-base --is-ancestor <hash> <trunk>`, НЕ по имени ветки/визуально — хеши меняются на rebase (a0d55f4→88508f2, e29a4a9→0369e7c).
> ЭТА версия файла — каноническая; старые копии в боковых ветках протухли.

---

## ✅ Закрыто (детали — CHANGELOG + память)

- **R — RELEASE 1.19.0** (`4ef45b2`, прод SW v85). Остаток за Gio: ручной тест «назад» на телефоне.
- **4c — DB-SPLIT Workouts** (`e3b2992`, bold-fermi, SW v87).
- **BUG-DRUM-0 — сет «0×1»** (релиз 1.19.1 `c39f33f`, на проде). Репро-тесты в `test/drum-picker.test.js`.
- **PERF-DRUM — виртуализация барабанов** (2026-07-04, `815eec7`+`f73a6f3`, awesome-davinci): окно 15 значений за флагом `drum-virtual` (default ON), спейсеры сохраняют scrollTop-математику, `_updateActive` адресно. Замеры: drum-items 4642→750 (-84%), DOM экрана 5479→1696. Все гарды BUG-DRUM-0 гоняются на обоих путях (юниты ×2). Гейт 229/229 + e2e 39/39. Kill switch: `Flags.setFlag('drum-virtual', false)`. Остаток за Gio: полевой чек скролла/фризов на телефоне.
- **4b — DB-SPLIT мелкие сторы** (`ae9ec9a`, determined-yalow): Metrics/Events/Nutrition/Planned в `js/db/*`. Влито merge queue 2026-07-04.
- **FS — Fast Skip онбординга** (`dd03202`+`0c62350`, vigorous-hypatia) + полевые визуальные фиксы. Влито merge queue 2026-07-04 — до этого НЕ было на проде.
- **BLACK-DRUM + GLASS-LITE (пара)** (`6c8c6e9`+`ac0f1e5`, trusting-antonelli): барабаны total-black вариант B; широкий GLASS-LITE откатан — снятие blur с карточек теперь по программе AIR (`HANDOFF_air_refactor.md`), карточки-дубли ниже удалены. `ac0f1e5` = чекпоинт `pre-air`.
- **AIR-0.5 — Token Heal** (`2d737dd`, wizardly-chatelet): фантомный `--c-surface-deep` → `var(--c-bg-2)`. Детали — `HANDOFF_air_refactor.md`.
- **Merge queue 2026-07-04 (LEAD)** — 5 линий собраны cherry-pick в одну прямую линию, FF в trunk; SW v87/v88/v89 параллельных веток разрулены → финал v90. Ветки yalow/fermi/davinci/wu/chatelet/hypatia/chandrasekhar/antonelli больше не нужны.
- **BLUR-SCROLL** — ✅ влит: `1e098fa` в trunk/main (проверено `merge-base --is-ancestor` 2026-07-05). Глобальный `body.is-scrolling *` snap-off снят (в `css/base.css` остался только комментарий), точечный `will-change` на всех 9 glass-поверхностях (summary/modal/toast/tab-bar/Island/intel/athlete-room). Класс `is-scrolling` в `js/app.js:355` остался, но безвреден — CSS на него больше не реагирует. Симптом «блур гаснет при скролле summary» закрыт этим одним правилом — отдельных багов по экранам не было.
- **CORE-TAP-SAFETY** — ✅ влит: `a0d55f4` → после rebase `88508f2`, в trunk/main. confirmDialog перед remove-X (паттерн `cancelSession`, `danger:true`) + `.core-list` margin-top +16px. Юнит-тест не добавлен — нет DOM-инфры для `workout.view/*` (handlers.js тянет document/IDB на импорте); DOM-шим = отдельная карточка при надобности.
- **STATS-1 — герой+рельс** — ✅ влит: `e29a4a9` → после rebase `0369e7c`, в trunk/main. live-bar → `.live-hero` (тоннаж count-up + сегментный рельс по сетам + чип ex). Детали — `HANDOFF_stats_redesign.md`.
- **MERGE-QUEUE-2 (LEAD, Sonnet 5)** — ✅ закрыта 2026-07-05: nervous-williamson + funny-tharp + silly-antonelli собраны в прямую линию → релиз **1.20.0 `bb980c8`** (SW v92), запушено в origin/main, прод редеплой. Полевой чек blur пройден Gio. Ветки-доноры больше не нужны.
- **UI-CENTER** — ✅ закрыта: `e9c7060` (serene-ishizaka) → в trunk после rebase (`4ebb5b3`): однострочный заголовок упражнения (handle·icon·имя·badge·кнопки·chevron на одной оси), счётчик X/Y ушёл в шапку сетов — вертикальный перекос кластера имени снят конструктивно.

## Карточка AIR-2 — Controls + Analytics 🔴 (следующая: вектор Gio, AIR-1 влита + полевой чек пройден)

- Полная карточка — `HANDOFF_air_refactor.md` § AIR-2: сегмент-группа `css/base.css` (week-segment, bs-sex-segment, stats-subnav, bs-tab-bar, plan-tabs) → рецепт AIR; active-состояния — плоский `--c-chrome-t` + hairline, PPL text-shadow week-seg оставить; остатки blur в analytics.css снять.
- **ГДЕ СТОП:** verify-блок, полевой чек, 1 коммит + SW-бамп (следующий свободный v93).
- **НЕ ТРОГАТЬ:** Tier 2, summary/intel/athlete-room (это AIR-3).

## Карточка GLOW-SL — свечение и просвечивания в set logger 🟠 (разблокирована: BLACK-DRUM влит)

- **СИМПТОМЫ (скрин + замеры 2026-07-03):** (а) соседние значения барабанов фантомно просвечивают возле чужих строк и ADD SET; (б) тёмный скрим/градиент наезжает на ADD SET при done-сетах; (в) статс-бар просвечивает сквозь раскрытый Island (alpha .85 + blur); (г) зелёное свечение вокруг `.set-check.done`.
- **ЦЕЛЬ:** `mask-image` fade на `.drum-track` вместо голой opacity (обрезать фантомы за пределами колеса), убрать наезд скрима на ADD SET, поднять непрозрачность раскрытого Island до ~0.95. Перепроверить (а) после BLACK-DRUM (opacity 0.4 меняет картину) и (в) после STATS-1 (статс-бар теперь `.live-hero` — симптом мог измениться/уйти).
- **ГДЕ СТОП:** скриншоты до/после; гейт.
- **НЕ ТРОГАТЬ:** island-set-pulse; геометрию барабана (ITEM_H/snap).

## Карточка A-4 — тест видимости drum после Add Set 🟨 (дёшево, страхует прод-фикс)

- **ЦЕЛЬ:** unit/e2e: после «+ add set» новый ряд имеет `.drum-item--active` с видимым числом (регресс BUG-7/drum-fix fd99a2e). Гонять с флагом `drum-virtual` в обоих положениях (паттерн — `test/drum-picker.test.js`).
- **ГДЕ СТОП:** тест падает на ревёрте фикса, зелёный на текущем коде.
- **НЕ ТРОГАТЬ:** сам set-logger.

## Карточка 4d — DB-SPLIT: Backup + финализация

- **ЦЕЛЬ:** вынести Backup (import/export, withMeta-штамп) в `js/db/backup.js`; `js/db.js` остаётся чистым фасадом. build:sw + бамп (следующий свободный номер — сверить с 4b!). Карточка 4 закрыта целиком → отметить в HANDOFF_air_cleanup.md.
- **ГДЕ СТОП:** гейт + смоук export→import в превью.
- **НЕ ТРОГАТЬ:** формат бэкап-JSON (совместимость со старыми экспортами и import-ETL).

## Карточка Q2 — PII console.log sweep (мелкая, можно делегировать)

- **ЦЕЛЬ:** убрать логирование чувствительного: `routes/integrations.js:69` (длины ключей G=/A=) + grep `console.log` по routes/lib/js на ключи/имена/веса → почистить или перевести на lib/logger.js.
- **ГДЕ СТОП:** grep не находит PII/ключей в логах. Функциональные логи не трогать.
- **НЕ ТРОГАТЬ:** логику; только лог-строки.

## Карточка SET-STALE — чек-боксы сетов не перерисовываются при внешнем апдейте State 🟦 (низкий: один пользователь/одна вкладка в поле)

- **СИМПТОМ:** если State.plan обновился не через клик в этой вкладке (вторая вкладка на том же origin / resume-гонка), live-стата пересчитывается (`_updateLiveStats` — тоннаж/рельс/чип верные), а `.set-check`/`.set-done-summary` в строках остаются старыми: наблюдалось State=19 done при 9 закрашенных чек-боксах в DOM (верификация STATS-1, 2026-07-05).
- **ЦЕЛЬ:** найти, кто обновляет State без перерисовки строк (sync/storage-listener? resume-путь?), и дорисовывать строки (точечный `renderSetRow` по расхождению, не полный re-render).
- **ГДЕ СТОП:** репро-сценарий двух вкладок задокументирован; после фикса чек-боксы == State.
- **НЕ ТРОГАТЬ:** формулы `_updateLiveStats`; LWW-механику sync.js.

## Карточка W — прополка правил (регулярная: раз в ~2 недели, после закрытия пакета; следующая ~середина июля 2026)

- **ЦЕЛЬ:** ревизия правил, не кода. По каждому жёсткому правилу (CLAUDE.md § Rules + Session Protocol, NEXT_SESSION § анти-хрупкий workflow, память-feedback): срабатывало за период → живёт; ни разу → вынести из жёстких в заметку или удалить. Обновить протухшие факты (версии, цифры тестов, имена файлов). Слить дубли: правило живёт в одном месте, остальные — ссылки.
- **ГДЕ СТОП:** один docs-коммит. Прополка — ЕДИНСТВЕННОЕ место, где правила меняются; между прополками они неприкосновенны.
- **НЕ ТРОГАТЬ:** код; правила с пометкой жёсткого запрета пользователя (island-set-pulse) — только с Gio.
- Прополка №1 = DOC-SYNC 2026-07-02 (сделана).

---

После стека: роадмап CRDT foundation + полевые DS1 (миграция v3→v4 на живых данных) / DS2 (2-девайс конвергенция) — за Gio с телефонами.
